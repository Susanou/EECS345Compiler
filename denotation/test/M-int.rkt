...
(define/provide-test-suite 
  ...
  (test-suite
   "integers"
   (test-equal? "zero"
                (M-int 0 null M-state)
                (success 0))
   (test-equal? "fize"
                (M-int 5 null M-state)
                (success 5))
   (test-equal? "negative three"
                (M-int -3 null M-state)
                (success -3)))
  (test-suite
   "variables"
   (test-equal? "ok"
   ...
   (test-equal? "unbound"
                (M-int 'x (machine-new) M-state)
                (failure "use before bind: x")))
  (test-suite
   "expressions"
   (test-equal? "plus"
                (M-int '(+ 1 2) null M-state)
                (success 3))
   (test-equal? "minus"
                (M-int '(- 1 2) null M-state)
                (success -1))
   (test-equal? "multiply"
                (M-int '(* 5 2) null M-state)
                (success 10))
   (test-equal? "divide"
                (M-int '(/ 5 2) null M-state)
                (success 2))
   ...
   (test-equal? "6 * (8 + (5 % 3)) / 11 - 9"
                (M-int '(- (/ (* 6 (+ 8 (% 5 3))) 11) 9) (machine-new) M-state)
                (success -4)
                )))
